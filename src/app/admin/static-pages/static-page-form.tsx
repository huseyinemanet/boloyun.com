...
export function StaticPageForm({ page, mode = "edit" }: { page: StaticPageRow; mode?: "create" | "edit" }) {
  const isEditing = mode === "edit";
  return (
    <form action={saveStaticPageAction} className="space-y-4">
      <input type="hidden" name="id" value={isEditing ? page.id : ""} />
      <section className="space-y-4 rounded-md border border-border bg-card p-4 md:p-5">
        ...
        <label className="block text-sm font-bold">
          İçerik
          <Textarea name="content" defaultValue={staticPageEditorContent(page)} rows={22} required className="mt-1 resize-y font-mono text-xs leading-6" placeholder={"Bölüm başlığı\nİlk paragraf\nİkinci paragraf\n\n---\n\nYeni bölüm başlığı\nParagraf"} />
      ...
      <section className="space-y-4 rounded-md border border-border bg-card p-4 md:p-5">
        ...
        {isEditing ? <div className="rounded-md border border-border bg-muted/30 p-3 text-xs"><p className="font-bold">Canonical URL</p><p className="mt-1 break-all text-muted-foreground">{absoluteUrl(`/sayfa/${page.slug}`)}</p></div> : null}
      </section>

      <div className="flex flex-wrap items-center justify-between gap-3">
        <Button asChild variant="outline"><Link href="/admin/static-pages">İptal</Link></Button>
        <div className="flex items-center gap-2">
          {isEditing && page.status === "published" ? <Button asChild variant="outline"><Link href={`/sayfa/${page.slug}`} target="_blank">Sayfayı Görüntüle <ExternalLinkIcon /></Link></Button> : null}
          <Button className="font-bold">{isEditing ? "Sayfayı Güncelle" : "Sayfayı Oluştur"}</Button>
        </div>
      </div>
    </form>
  );
