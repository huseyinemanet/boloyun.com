...
test("production build uses the standalone Node runtime", async () => {
  const [nextConfig, packageJson, compose, deploy] = await Promise.all([
    readFile(new URL("../../next.config.ts", import.meta.url), "utf8"),
    readFile(new URL("../../package.json", import.meta.url), "utf8"),
    readFile(new URL("../../deploy/compose.yml", import.meta.url), "utf8"),
    readFile(new URL("../../deploy/server/boloyun-deploy", import.meta.url), "utf8"),
  ]);
  ...
  assert.match(compose, /boloyun-app/);
  assert.match(compose, /boloyun-worker/);
